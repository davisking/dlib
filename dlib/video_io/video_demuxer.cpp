...
namespace dlib
{
    ...
    bool decoder_extractor::is_open() const noexcept
    {
        ...
        const bool object_alive  = pCodecCtx != nullptr && opened;
    ...
    typedef enum {
        EXTRACT_SEND_PACKET,
        EXTRACT_READ_FRAME_DONE,
        EXTRACT_READ_FRAME_SEND_PACKET,
        EXTRACT_DONE,
        EXTRACT_ERROR = -1
    } extract_state;
    ...
    {
        auto send_packet = [&](extract_state& state)
        {
            const int ret = avcodec_send_packet(pCodecCtx.get(), pkt.get());

            if (ret >= 0) {
                state   = EXTRACT_READ_FRAME_DONE;
            } else if (ret == AVERROR(EAGAIN)) {
                state   = EXTRACT_READ_FRAME_SEND_PACKET;
        ...
        auto recv_frame = [&](extract_state& state, bool resend)
        {
        ...
        extract_state state = opened ? EXTRACT_SEND_PACKET : EXTRACT_ERROR;
        ...
        while (state != EXTRACT_ERROR && state != EXTRACT_DONE)
        {
            switch(state)
            {
                ...
                case EXTRACT_READ_FRAME_DONE:          recv_frame(state, false);   break;
                case EXTRACT_READ_FRAME_SEND_PACKET:   recv_frame(state, true);    break;
                default: break;
            }
        }
    ...
    decoder_read_t decoder_extractor::read(Frame &dst_frame)
    {
    ...
    decoder_read_t decoder_extractor::read(
    ...
    )
    {
        ...
        if (suc == DECODER_FRAME_AVAILABLE)
        {
            if (f.is_image())
            {
    ...
    bool            decoder_ffmpeg::is_open()           const noexcept { return extractor.is_open(); }
    bool            decoder_ffmpeg::is_image_decoder()  const noexcept { return extractor.is_image_decoder(); }
    bool            decoder_ffmpeg::is_audio_decoder()  const noexcept { return extractor.is_audio_decoder(); }
    AVCodecID       decoder_ffmpeg::get_codec_id()      const noexcept { return extractor.get_codec_id(); }
    std::string     decoder_ffmpeg::get_codec_name()    const noexcept { return extractor.get_codec_name(); }
    int             decoder_ffmpeg::height()            const noexcept { return extractor.height(); }
    int             decoder_ffmpeg::width()             const noexcept { return extractor.width(); }
    AVPixelFormat   decoder_ffmpeg::pixel_fmt()         const noexcept { return extractor.pixel_fmt(); }
    int             decoder_ffmpeg::sample_rate()       const noexcept { return extractor.sample_rate(); }
    uint64_t        decoder_ffmpeg::channel_layout()    const noexcept { return extractor.channel_layout(); }
    AVSampleFormat  decoder_ffmpeg::sample_fmt()        const noexcept { return extractor.sample_fmt(); }
    int             decoder_ffmpeg::nchannels()         const noexcept { return extractor.nchannels(); }
    ...
    bool decoder_ffmpeg::push_encoded_padded(const uint8_t *encoded, int nencoded)
    {
        if (!is_open())
            return false;

        decoder_state state     = DECODE_RECV_PACKET;
        ...
        auto recv_packet = [&]
        {
            ...
            if (packet->size || is_flushing)
                state = DECODE_SEND_PACKET;
        };

        auto send_packet = [&]
        {
            if (extractor.push(packet))
                state = DECODE_RECV_PACKET;
            else
                state = DECODE_DONE;
        };

        while (state != DECODE_DONE && state != DECODE_ERROR)
        {
            switch(state)
            {
                case DECODE_RECV_PACKET: recv_packet(); break;
                case DECODE_SEND_PACKET: send_packet(); break;
                default: break;
            }
        }

        return state != DECODE_ERROR;
    }
    ...
    decoder_read_t decoder_ffmpeg::read(Frame& dst_frame)
    {
        return extractor.read(dst_frame);
    }

    decoder_read_t decoder_ffmpeg::read(
        type_safe_union<array2d<rgb_pixel>, audio_frame> &frame,
        std::chrono::system_clock::time_point &timestamp
    )
    {
        return extractor.read(frame, timestamp);
    }
